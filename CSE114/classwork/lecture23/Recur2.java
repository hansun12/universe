public class Recur2 {
    ...
    public static int sumTo(int n) {
        if (n == 0) {   // Base case
	    System.out.println("sumTo(0) : Done!");
            return 0;
        } else {
	    System.out.println(n + " + sumTo(" + n + " - 1)");
            return n + sumTo(n - 1);
        }
    }
    ...
    public static int sumSquareTo(int n) {
        if (n == 0) {   // Base case
	    System.out.println("sumSquareTo(0) : Done!");
            return 0;
        } else {        // recursion
	    System.out.println(square(n) + " + sumSquareTo(" + n + " - 1)");
    ...
    public static int sumEvenSquareOddCubeTo(int n) {
	    ...
	    System.out.println(square(n) + "[square("+n+")] + sumEvenSquareOddCube(" + n + " - 1)");
            return square(n) + sumEvenSquareOddCubeTo(n - 1);
        } else {
	    System.out.println(cube(n) + "[cube(" + n + ")] + sumEvenSquareOddCube(" + n + " - 1)");
	...
	System.out.println("Calling: concatTo2Aux(0, " + n + ")");
        return concatTo2Aux(0, n);
    }

    private static String concatTo2Aux(int from, int to) {
        if (from > to) {
	    System.out.println("concatTo2Aux: Base: Returning: \"\"");
            return "";
        } else {
	    System.out.println("concatTo2Aux(" + (from + 1) + ", " + to + ")");
	...
	System.out.println("Calling: concatTo3Aux(0, " + n + ", \"\")");
    ...
    private static String concatTo3Aux(int from, int to, String result) {
        if (from > to) {
	    System.out.println("concatTo3Aux: Base: Returning: " + result);
            return result;
        } else {
	    System.out.println("concatTo3Aux(" + (from + 1) + ", " + to + ", \"" + result+from + "\")");
